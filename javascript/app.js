/* ===== Violet Gloss — Core App (non-module, NO imports) ===== */
/* This file contains ALL non-Firebase functionality.
   It loads and runs even if Firebase CDN is blocked. */
...
function initCustomSelects() {
    ...
    var selects = document.querySelectorAll('select[data-vg-select], select:not([data-no-vg])');
    ...
    selects.forEach(function(select) {
        ...
        function buildOptions() {
            ...
            Array.from(select.options).forEach(function(option, idx) {
                ...
                optDiv.addEventListener('click', function(e) {
                    ...
                    dropdown.querySelectorAll('.vg-select-option').forEach(function(o) { o.classList.remove('selected'); });
        ...
        trigger.addEventListener('click', function(e) {
            ...
            document.querySelectorAll('.vg-select.open').forEach(function(s) {
        ...
        select.addEventListener('change', function() {
            ...
            if (opt) {
                ...
                dropdown.querySelectorAll('.vg-select-option').forEach(function(o) {
    ...
    document.addEventListener('click', function(e) {
        if (!e.target.closest('.vg-select')) {
            document.querySelectorAll('.vg-select.open').forEach(function(s) { s.classList.remove('open'); });
        }
    });
}
...
function applyLanguage(lang) {
    ...
    document.querySelectorAll('.vg-select').forEach(function(vs) {
        var selectId = vs.dataset.selectId;
        if (selectId) {
            var nativeSelect = document.getElementById(selectId);
            if (nativeSelect) {
                var selectedOpt = nativeSelect.options[nativeSelect.selectedIndex];
                var valueSpan = vs.querySelector('.vg-select-value');
                if (valueSpan && selectedOpt) {
                    valueSpan.textContent = selectedOpt.textContent;
                }
                // Rebuild options in the custom dropdown
                var dropdown = vs.querySelector('.vg-select-dropdown');
                if (dropdown) {
                    dropdown.querySelectorAll('.vg-select-option').forEach(function(optDiv, idx) {
...
function initLanguage() {
    var saved = localStorage.getItem('vg-language') || 'ru';
    applyLanguage(saved);
}
...
window.t = function(key) {
    var lang = localStorage.getItem('vg-language') || 'ru';
    var t = translations[lang] || translations.en;
    return t[key] || key;
};
